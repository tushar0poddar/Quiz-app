...
const scoreFeedback = (userScore) => {
  ...
  if (userScore >= 20 && userScore <= 25) {
    confettiStart();
    resultFeedback.textContent = `Well done on your outstanding performance. Keep up the good work and continue to showcase your Knowledge.`;
  } else if (userScore >= 15 && userScore < 20) {
    confettiStart();
    resultFeedback.textContent = `You have a good Knowledge about your college. Your performance is good, but with additional practice and focus, you can excel in this subject. `;
  } else {
    confetti.clear();
    resultExpression.textContent = `Needs Improvement!!!`;
    resultFeedback.textContent = `The result of this quiz is not satisfactory, it's clear that you have a limited knowledge about your college.`;
  }
};
